 ...
 *   image    -> chooseImage / previewImage（Issue #9 image 字段）
 *
 * storage 用 Map 实现，保证读写在测试内可见、可断言；
 * request/showToast/reLaunch/chooseImage/previewImage 用空实现，
 * 由各测试用 vi.mocked(...) 自行配置返回。
 */
...
const uniStub = {
  ...
  showToast: vi.fn(),
  reLaunch: vi.fn(),
  // Issue #9：image 字段调 uni.chooseImage 选图
  chooseImage: vi.fn(),
  // Issue #9：image 字段调 uni.previewImage 预览
  previewImage: vi.fn(),
}
